...
namespace WEngine {
    ...
    void CameraComponent::LateUpdate(float deltaTime) {
        ...
        glm::vec3 movement = GetMovementInput(deltaTime);
        glm::vec3 rotation = GetRotationInput(deltaTime);
        if (movement != glm::vec3{0.0f} || rotation != glm::vec3{0.0f}) {
            ILogger *logger = ServiceLocator::GetService<ILogger *>(typeid(ILogger).name());
            logger->Log("movement: " + glm::to_string(movement) + ", rotation: " + glm::to_string(rotation) + "\n");
        }
    ...
    glm::vec3 CameraComponent::GetMovementInput(float deltaTime) {
        glm::vec3 movement{0.0f};

        const float cameraSpeed{moveSpeed * deltaTime};
        // camera z-axis is pointing out from the screen
        if (InputSystem::Instance()->KeyHold(GLFW_KEY_W)) {
        ...
        if (InputSystem::Instance()->KeyHold(GLFW_KEY_Z)) {
            movement.y += cameraSpeed;
        }
        if (InputSystem::Instance()->KeyHold(GLFW_KEY_C)) {
            movement.y -= cameraSpeed;
        }
    ...
    glm::vec3 CameraComponent::GetRotationInput(float deltaTime) {
        glm::vec3 rotation{0.0f};
        // keyboard inputs
        if (InputSystem::Instance()->KeyHold(GLFW_KEY_KP_4)) {
            rotation.y += yawSpeed * deltaTime;
        }
        if (InputSystem::Instance()->KeyHold(GLFW_KEY_KP_6)) {
            rotation.y -= yawSpeed * deltaTime;
        }
        if (InputSystem::Instance()->KeyHold(GLFW_KEY_KP_8)) {
            rotation.x += pitchSpeed * deltaTime;
        }
        if (InputSystem::Instance()->KeyHold(GLFW_KEY_KP_2)) {
            rotation.x -= pitchSpeed * deltaTime;
        }
        if (InputSystem::Instance()->KeyHold(GLFW_KEY_KP_7)) {
            rotation.z += rollSpeed * deltaTime;
        }
        if (InputSystem::Instance()->KeyHold(GLFW_KEY_KP_9)) {
            rotation.z -= rollSpeed * deltaTime;
        }
        ...
        rotation.y += mouseDelta.first * yawSpeed * deltaTime;
        rotation.x += mouseDelta.second * pitchSpeed * deltaTime;
        return rotation;
    }
